...
export type PaletteGeneratedSection = {
...
export type PaletteSectionModel = {
  ...
  footerText?: string;
  generated?: PaletteGeneratedSection;
};
...
export function PaletteSectionBody({
  ...
  if (section.generated?.html || section.generated?.css) {
    return <GeneratedSectionSurface section={section} />;
  }
...
function GeneratedSectionSurface({ section }: { section: PaletteSectionModel }) {
  const hostRef = useRef<HTMLDivElement | null>(null);
  const html = previewHtmlForSection(section, section.generated?.html ?? "");
  const css = section.generated?.css ?? "";
  ...
  useEffect(() => {
    ...
    root.innerHTML = `
      ...
      <style>
        ...
        .palette-reference-fallback {
          overflow: hidden;
          border-radius: 28px;
          min-height: clamp(220px, 32vw, 430px);
          background: #d8c09b;
        }
        .palette-generated-fallback {
          display: grid;
          gap: 18px;
      ...
      ${html}
    `;
    if (imageUrl) applyReferenceImageToShadow(root, imageUrl, imageAlt);
  }, [css, html, imageAlt, imageUrl]);

  return (
    <div
      ref={hostRef}
      className="generated-custom-surface"
      aria-label={`${section.kind} preview generated by Palette`}
    />
  );
}

function previewHtmlForSection(section: PaletteSectionModel, html: string) {
  const candidate = html.trim();
  if (/<[a-z][\s/>]/i.test(candidate)) return candidate;
  return [
    `<section class="palette-generated-fallback palette-generated-fallback-${escapeHtml(section.kind)}">`,
    section.eyebrow ? `<p>${escapeHtml(section.eyebrow)}</p>` : "",
    `<h2>${escapeHtml(section.title)}</h2>`,
    section.subtitle ? `<p>${escapeHtml(section.subtitle)}</p>` : "",
    section.imageUrl
      ? `<figure class="palette-reference-fallback palette-reference-host"><img class="palette-reference-image" src="${escapeHtml(section.imageUrl)}" alt="${escapeHtml(section.imageAlt ?? section.title)}" /></figure>`
      : "",
    "</section>",
  ].join("");
}
...
function applyReferenceImageToShadow(root: ShadowRoot, url: string, alt: string) {
  const existingImage = root.querySelector<HTMLImageElement>("img");
  if (existingImage) {
    existingImage.src = url;
    existingImage.alt = alt;
    existingImage.classList.add("palette-reference-image");
    return;
  }
  ...
  const target =
    candidates.find((element) => !element.querySelector("h1,h2,h3,p,a,button,input,textarea,select") && !element.textContent?.trim()) ??
    candidates[0];

  const image = document.createElement("img");
  image.src = url;
  image.alt = alt;
  image.className = "palette-reference-image";

  if (target) {
    target.replaceChildren(image);
    target.classList.add("palette-reference-host");
    return;
  }

  const figure = document.createElement("figure");
  figure.className = "palette-reference-fallback palette-reference-host";
  figure.append(image);
  root.append(figure);
}
